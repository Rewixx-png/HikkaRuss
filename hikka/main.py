...
class Hikka:
    ...
    async def _web_banner(self):
        ...
        logging.info("🌐 Пожалуйста, посетите %s", self.web.url)
    ...
    async def _phone_login(self, client: CustomTelegramClient) -> bool:
        phone = input(
            "\033[0;96mВведите номер телефона: \033[0m"
        ...
        if not self.web:
            ...
            print(
                (
                    ...
                    "Вы можете использовать QR-код для входа с другого устройства (например, телефона друга)."
                )
            )
            ...
            if qr_logined:
                ...
                while True:
                    ...
                    try:
                        ...
                        print(
                            "\033[0;91mВы получили FloodWait ошибку! Пожалуйста, подождите"
            ...
            print("\033[0;92mУспешный вход!\033[0m")
            await self.save_client_session(client)
            self.clients += [client]
            return True
    ...
    async def _init_clients(self) -> bool:
        ...
        for session in self.sessions.copy():
            try:
                ...
                logging.error(
                    (
                        "Убедитесь, что это единственный запущенный экземпляр. "
                        "Если это не помогает, удалите файл '%s'"
                    ),
                    session.filename,
                )
                continue
                ...
                logging.error(
                    "Неверный номер телефона. Используйте международный формат (+XX...) "
                    "и не вставляйте пробелы."
                )
                self.sessions.remove(session)
            except InteractiveAuthRequired:
                logging.error(
                    "Сессия %s была завершена и требуется повторная авторизация",
                    session.filename,
                )
                self.sessions.remove(session)
    ...
    async def _badge(self, client: CustomTelegramClient):
        ...
        try:
            ...
            upd = "Требуется обновление" if diff else "Обновлений нет"

            logo = (
                "█ █ █ █▄▀ █▄▀ ▄▀█\n"
                "█▀█ █ █ █ █ █ █▀█\n\n"
                f"• Билд: {build[:7]}\n"
                f"• Версия: {'.'.join(list(map(str, list(__version__))))}\n"
                f"• {upd}\n"
            )
